...
const sidebarLinks = [
  ...
  { to: '/staff/marks', label: 'Marks', icon: '📝' },
  { to: '/staff/timetable', label: 'Timetable', icon: '🗓️' },
  { to: '/staff/notices', label: 'Notices', icon: '📢' },
      ...
      >
        ...
        <div className="px-6 py-5 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-cyan-500 to-teal-600 flex items-center justify-center text-xl shadow-lg shadow-cyan-500/20">
              🎓
            </div>
            <div>
              <span className="text-lg font-bold text-white tracking-tight block leading-tight">EdTech</span>
              <span className="text-[11px] text-slate-500 font-medium uppercase tracking-wider">Staff Portal</span>
            </div>
          </div>
          <button
            onClick={() => setIsOpen(false)}
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `flex items-center gap-3 px-4 py-2.5 rounded-xl text-sm font-medium transition-all duration-200 ${
                    isActive
                      ? 'bg-cyan-600/15 text-cyan-400 shadow-sm'
